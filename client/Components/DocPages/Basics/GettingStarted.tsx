...
import { ObsidianRouter, gql } from 'https://deno.land/x/obsidian/mod.ts';`}
      ...
      <p>
        First, make sure you've imported all necessary schemas, types and resolvers.  When you have everything you need to create a GraphQL endpoint, you can do so using
        the following code  (Note that the router should come <i>before</i> your{' '}
...
app.use(GraphQLRouter.routes(), GraphQLRouter.allowedMethods());`}
      ...
      <p>
        It's that easy! Keep in mind that the features we're using are still pretty new,
        so we're going to have to run the server with the --unstable flag.  The command to start up our example server would look like this:
      </p>
      <p>
